...
export const ImportResultsDialog: React.FC<ImportResultsDialogProps> = ({
  ...
  const isProcessing = importStep === 'uploading' || importStep === 'processing';

  return (
    <Dialog.Root open={isOpen} onOpenChange={(open) => !open && handleClose()}>
      <Dialog.Portal>
        <Dialog.Overlay className="fixed inset-0 bg-black/50 z-50" />
        <Dialog.Content className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-50 bg-neutral-800 rounded-lg border border-neutral-800 p-6 w-96 max-h-[80vh] overflow-y-auto focus:outline-none">
          <div className="flex items-center justify-between mb-4">
            <Dialog.Title className="text-lg font-medium text-neutral-200">Import Results</Dialog.Title>
          ...
          {importStep === 'select' && (
            <div className="space-y-4">
              ...
              <div className="flex gap-3 mt-6">
                ...
                <Dialog.Close asChild>
                  <button className="px-4 py-2 bg-neutral-700 text-neutral-300 rounded hover:bg-neutral-600 transition-colors">
                    Cancel
                  </button>
                </Dialog.Close>
              </div>
